...
const getApiBaseUrl = () => {
  // Toujours utiliser les routes API locales (/api) pour Vercel + Neon
  return '/api';
};
...
export const dashboardApi = {
  ...
  getReport: (type: string, year?: number)      => api.get(`/rapports/${type}`, { params: { year: year ?? new Date().getFullYear() } }).then((r) => r.data),
};
